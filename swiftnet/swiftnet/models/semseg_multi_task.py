...
from swiftnet.models.util import _BNReluConv, upsample


class SemsegModelMultiTask(nn.Module):
    def __init__(self, backbone, num_classes1, num_classes2, num_inst_classes=None, use_bn=True, k=1, bias=True,
                 loss_ret_additional=False, upsample_logits=True, logit_class=_BNReluConv,
                 multiscale_factors=(.5, .75, 1.5, 2.)):
        super(SemsegModelMultiTask, self).__init__()
        ...
        self.logits1 = logit_class(self.backbone.num_features, self.num_classes1, batch_norm=use_bn, k=k, bias=bias)
        self.logits2 = logit_class(self.backbone.num_features, self.num_classes2, batch_norm=use_bn, k=k, bias=bias)
    ...
    def forward(self, image, target_size, image_size):
        ...
        logits1 = self.logits1.forward(features)
        logits2 = self.logits2.forward(features)
        if (not self.training) or self.upsample_logits:
            #logits = upsample(logits, image_size)
            logits1 = upsample(logits1, image_size)
            logits2 = upsample(logits2, image_size)
        if hasattr(self, 'border_logits'):
            additional['border_logits'] = self.border_logits(features).sigmoid()
        #additional['logits'] = logits
        additional['logits1'] = logits1
        additional['logits2'] = logits2
        return logits1, logits2, additional
    ...
    def do_forward(self, batch, image_size=None):
        ...
        logits1, logits2,  additional = self.forward(**data)
    ...
    def loss(self, batch):
        ...
        assert self.criterion1 is not None and self.criterion2 is not None
        #labels = batch['labels'].cuda()
        labels1 = batch['labels1'].cuda()
        labels2 = batch['labels2'].cuda()
        #logits, additional = self.do_forward(batch, image_size=labels.shape[-2:])
        logits1, logits2, additional = self.do_forward(batch, image_size=labels1.shape[-2:])
        if self.loss_ret_additional:
            #return self.criterion(logits, labels, batch=batch, additional=additional), additional
            return self.criterion1(logits1, labels1, batch=batch, additional=additional) + \
                   self.criterion2(logits2, labels2, batch=batch, additional=additional), additional
        #return self.criterion(logits, labels, batch=batch, additional=additional)
        return self.criterion1(logits1, labels1, batch=batch, additional=additional) + \
               self.criterion2(logits2, labels2, batch=batch, additional=additional)

        
    def random_init_params(self):
        params = [self.logits1.parameters(), self.logits2.parameters(), self.backbone.random_init_params()]
        if hasattr(self, 'border_logits'):
