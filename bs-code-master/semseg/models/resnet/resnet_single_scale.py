...
from semseg.models.util import _Upsample, SpatialPyramidPooling, SeparableConv2d
from semseg.evaluation.evaluate import mt
...
class ResNet(nn.Module):
    def __init__(self, block, layers, *, num_features=128, k_up=3, efficient=False, use_bn=True,
        ...
        self.img_mean_override = torch.tensor(mean).view(1, -1, 1, 1)
        self.img_std_override = torch.tensor(std).view(1, -1, 1, 1)
    ...
    def forward_down(self, image):
        ...
        # # print max element of tensor
        # print(image.max())
        # # show 3d tensor with matplotlib
        ...
        if self.img_mean_override != self.img_mean.detach().cpu().numpy():
            mean_on_cuda = self.img_mean_override.to(image.device)
            std_on_cuda = self.img_std_override.to(image.device)
            image -= mean_on_cuda
            image /= std_on_cuda
        else:
            image -= self.img_mean
