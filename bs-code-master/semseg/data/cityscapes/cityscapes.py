...
from semseg.data.cityscapes.labels import labels
...
class Cityscapes(Dataset):
    ...
    def __init__(self, root: Path, transforms: lambda x: x, subset='train', open_depth=False, labels_dir='gtFine', epoch=None, format="png", map_style=True):
        self.root = root
        ...
        self.images = list(sorted(self.images_dir.glob(f'*/*.{format}')))
        if self.has_labels:
            self.labels = list(sorted(self.labels_dir.glob(f'*/*labelIds.{format}')))
    ...
    def __getitem__(self, item):
        ...
        if self.map_style:
            ...
            return ret_dict['image'], ret_dict['labels']
